        ...
        <div className={`flex items-center gap-2 ${isMobile ? 'w-full' : 'justify-end'}`}>
            {user.isActive ? (
                <button
                    ...
                    className={`text-xs text-red-400 hover:text-red-300 font-medium transition-colors flex items-center gap-1 ${isMobile ? 'flex-1 justify-center py-1.5 rounded-lg border border-red-500/20 bg-red-500/10' : ''}`}
                >
                    <Ban size={13} /> Suspend
                </button>
            ) : (
            ...
            <button
                ...
                className={`text-xs text-white/30 hover:text-red-400 font-medium transition-colors flex items-center gap-1 ${isMobile ? 'p-1.5 rounded-lg hover:bg-red-500/10' : ''}`}
            >
...
export default function UsersTab() {
    ...
    const handleRefresh = async () => {
        ...
        setSelectedIds([]);
        try { await dispatch(listUsers()); } finally { setIsRefreshing(false); }
    };
    ...
    const filteredAndSortedUsers = useMemo(() => {
        ...
        switch (sortBy) {
            ...
            case 'orders_high': return sorted.sort((a, b) => (b.orderCount || 0) - (a.orderCount || 0));
            case 'orders_low': return sorted.sort((a, b) => (a.orderCount || 0) - (b.orderCount || 0));
    ...
    const handleSuspend = useCallback((user) => {
        setConfirm({
            title: 'Suspend User',
            message: `Suspend ${user.firstName} ${user.lastName}? They will lose access to the platform.`,
            confirmLabel: 'Suspend',
            confirmVariant: 'destructive',
            onConfirm: () => { dispatch(updateUserStatus({ userId: user._id, isActive: false })); setConfirm(null); }
        });
    }, [dispatch]);
    ...
    const handleActivate = useCallback((user) => {
        setConfirm({
            ...
            onConfirm: () => { dispatch(updateUserStatus({ userId: user._id, isActive: true })); setConfirm(null); }
        });
    }, [dispatch]);
    ...
    const handleBulkAction = useCallback((action) => {
        ...
        setConfirm({
            ...
            confirmLabel: label,
            confirmVariant: 'destructive',
            onConfirm: () => { dispatch(bulkUserAction({ userIds: selectedIds, action })); setSelectedIds([]); setConfirm(null); }
        });
            ...
            <PageLayout
                ...
                toolbar={
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                        <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg px-3 py-2 flex-1 focus-within:border-primary/40 transition-colors">
                        ...
                        <div className="relative">
                            ...
                            <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none">{getSortIcon()}</div>
                        </div>
                    </div>
                }
            >
                ...
                {selectedIds.length > 0 && (
                    <div className="mb-4 bg-primary/10 border border-primary/20 px-4 py-3 rounded-xl flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            <Button onClick={() => handleBulkAction('suspend')} variant="outline" size="sm" leftIcon={<Ban size={13} />}>Suspend</Button>
                            <Button onClick={() => handleBulkAction('delete')} variant="destructive" size="sm" leftIcon={<Trash2 size={13} />}>Delete</Button>
                        </div>
                    </div>
                )}
                ...
                {!loading && !error && filteredAndSortedUsers.length === 0 && (
                    <div className="text-center py-20 bg-secondary/30 rounded-2xl border border-dashed border-white/10">
                        ...
                        {searchQuery && (
                            <button onClick={() => setSearchQuery('')} className="mt-2 text-xs text-primary hover:text-primary/80">Clear search</button>
                        )}
                    </div>
                )}
                ...
                {!loading && filteredAndSortedUsers.length > 0 && (
                    <>
                            ...
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {filteredAndSortedUsers.map(user => (
                                        <tr key={user._id} className={`hover:bg-white/5 transition-all ${selectedIds.includes(user._id) ? 'bg-primary/5' : ''}`}>
                                            <td className="px-5 py-4">
                                                <input type="checkbox" checked={selectedIds.includes(user._id)} onChange={() => handleSelect(user._id)} className="accent-primary" />
                                            </td>
                                            <td className="px-5 py-4">
                        ...
                        <div className="md:hidden space-y-3">
                            {filteredAndSortedUsers.map(user => (
                                <div key={user._id} className={`bg-secondary/30 border rounded-2xl p-4 space-y-3 transition-all ${selectedIds.includes(user._id) ? 'border-primary/30 bg-primary/5' : 'border-white/10'}`}>
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="flex items-start gap-3">
                                            <input type="checkbox" checked={selectedIds.includes(user._id)} onChange={() => handleSelect(user._id)} className="accent-primary mt-0.5" />
                                            <div>
                                                <div className="text-white font-medium text-sm">{`${user.firstName || ''} ${user.lastName || ''}`.trim() || '—'}</div>
                                    ...
                                    <div className="flex items-center gap-4 text-xs">
                                        <div className="flex items-center gap-1.5">
                                            <ShoppingBag size={12} className="text-primary/60" />
                                            <span className="text-white font-medium">{user.orderCount ?? 0}</span>
                                            <span className="text-white/35">orders</span>
                                        </div>
